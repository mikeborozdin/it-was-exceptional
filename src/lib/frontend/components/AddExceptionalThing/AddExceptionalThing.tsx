...
import { SavePlaceInput } from "@/lib/backend/place/place";
...
const useServerActionMutate = (
  savePlaceFn: (input: SavePlaceInput) => Promise<string>
) => {
  ...
  const mutate = async (
    ...
    try {
      ...
      const newResult = await savePlaceFn({ ...input, photos });
...
interface Props {
  savePlace: (input: SavePlaceInput) => Promise<string>;
}

const AddExceptionalThing: React.FC<Props> = ({ savePlace }) => {
